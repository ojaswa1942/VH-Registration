<?php
	...
	if (mysqli_num_rows($result) == 1) {
		$sql = "UPDATE requests SET app_student=1 WHERE id='$id'";
		mysqli_query($db, $sql);

		$mail = new PHPMailer();                              // Passing `true` enables exceptions
		$email = '<strong>'.$row['st_name'].'</strong> has requested a '.$row['category'].' room for '.$row['first_name'].'. Click the following link to approve the request. <br /> <a href="http://127.0.0.1/git/VH-Registration/approve.php?key='.$row['st_key'].'&id='.$row['id'].'">http://127.0.0.1/git/VH-Registration/approve.php?key='.$row['st_key'].'&id='.$row['id'].'</a> .';
		$altemail = $row['st_name'].' has requested a '.$row['category'].' room for '.$row['first_name'].'. Open the following link in browser to approve the request. http://127.0.0.1/git/VH-Registration/approve.php?key='.$row['st_key'].'&id='.$row['id'].' .';
		try {
		    //Server settings
